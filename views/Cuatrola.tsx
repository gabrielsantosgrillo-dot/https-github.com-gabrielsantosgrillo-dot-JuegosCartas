...
const Cuatrola: React.FC<CuatrolaProps> = ({ onBack }) => {
  ...
  // Importante: isActionInProgress ahora es un ESTADO para que la IA reaccione cuando se desbloquea
  ...
  const checkAvailableCantos = useCallback((hand: Card[], player: PlayerPosition, currentTrump: Suit): PendingCanto[] => {
    ...
    suits.forEach(s => {
      ...
      // VERIFICACIÓN CRÍTICA: Deben estar ambas cartas físicamente en la mano
  ...
  const performCanto = useCallback((canto: PendingCanto) => {
    ...
    setTimeout(() => {
      ...
      if (completedTricksRef.current === 5) {
        ...
        // Importante: Forzamos el turno al ganador para que si es IA, el useEffect la dispare
  ...
  const playCard = useCallback((card: Card, player: PlayerPosition) => {
    if (currentPlayer !== player || isActionInProgress) return;
    ...
    if (newTrick.length === expectedSize) {
      setTimeout(() => {
        ...
        // REGLA CLAVE: Revisamos la mano después de que la carta ya no está
  ...
  }, [currentPlayer, currentTrick, trumpSuit, soloPlayer, getNextPlayer, checkAvailableCantos, performCanto, finalizeHandScore, isActionInProgress]);
  ...
  useEffect(() => {
    // La IA ahora reacciona a isActionInProgress para evitar bloqueos
    if (gamePhase === 'playing' && currentPlayer !== 'bottom' && !isActionInProgress) {
      const timer = setTimeout(() => {
        const currentHand = hands[currentPlayer];
        if (!currentHand || currentHand.length === 0) return;
        const playable = getPlayableCards(currentHand, currentTrick, trumpSuit);
        if (playable.length > 0) {
          playCard(playable.sort((a,b) => b.hierarchy - a.hierarchy)[0], currentPlayer);
        }
      }, 800);
      return () => clearTimeout(timer);
    }
  }, [gamePhase, currentPlayer, currentTrick, hands, trumpSuit, playCard, isActionInProgress]);
  ...
  const PlayerLabel = ({ position }: { position: PlayerPosition }) => {
    ...
    return (
      <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border transition-all ${isCurrent ? 'bg-emerald-500/40 border-emerald-400 scale-110 shadow-lg' : 'bg-slate-900/60 border-white/5'}`}>
        ...
        {isDealer && <span className="text-emerald-400 text-[6px] font-black italic underline ml-1">DA</span>}
      </div>
    );
  };
        ...
        <div className="w-full flex flex-col items-center gap-6 z-20 shrink-0">
          ...
          {humanCanCanto.length > 0 && (
            <div className="flex gap-4 mb-2 animate-bounce">
              ...
              <button onClick={() => { 
                setHumanCanCanto([]); 
                if(completedTricksRef.current === 5) finalizeHandScore(currentPlayer, handScoreRef.current); 
                else { setCurrentPlayer(manoPlayer); setIsActionInProgress(false); }
              }} className="px-5 py-2.5 bg-slate-800 text-white font-black text-[10px] rounded-full uppercase">PASAR</button>
            </div>
          )}
